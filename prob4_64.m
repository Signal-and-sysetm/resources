...
clear all
 ...
 %% ========== 
 T=.1;  M=28;  N=160; 
 wft=pi*[-1.5:.001:1.5];  Xft=2*sin(wft)./wft; 
 wdtfs=(2*pi/(N*T))*[-N/2:N/2-1]; 
 x=ones(1,21);  Xdtfs=fftshift(fft(x,N))*T; 

 figure(1),clf 
 subplot(2,1,1) 
 plot(wft/(2*pi),abs(Xft)) %% FT
 ,hold on,
 stem(wdtfs/(2*pi),abs(Xdtfs)) %%DFT
 ...
 %% Part (b) : 
 %% ========== 
 T=.9;  M=4;  N=56;  n=[-(M-1):M-1]; 
 wft=[-3:.001:3];  Xft=exp(-(wft).^2/2)/(sqrt(2*pi)); 
 wdtfs=(2*pi/(N*T))*[-N/2:N/2-1]; 
 x=exp(-(n*T).^2/2)/(2*pi);  Xdtfs=fftshift(fft(x,N))*T; 

 subplot(2,1,2) 
 plot(wft/(2*pi),abs(Xft)),hold on,stem(wdtfs/(2*pi),abs(Xdtfs)) 
 ...
 x=cos(20*pi*n*T) + cos(21*pi*n*T);  Xdtfs=fftshift(fft(x,M))/M; 
 ...
 subplot(2,1,2) 
 plot(wft/(2*pi),abs(Xft)),hold on,stem(wdtfs/(2*pi),abs(Xdtfs)) 
 title('P4.64(d)'),xlabel('Freq(Hz)'),ylabel('Part(d):|Y[k]|') 
 axis([9.5 11 0 .6]) 
