 ...
 %% Note : All dB plots are normalized with respect to the absolute maximum. 
 ...
 xlabel('Omega'),ylabel('|W_h(Omega)| dB:pi/50'),axis([-pi pi -130 0]) 
 ...
 xlabel('Omega'),ylabel('|W_h(Omega)| dB:pi/100'),axis([-pi pi -130 0]) 
 ...
 subplot(3,1,3) 
 plot(Omega,20*log10(abs(Wh)/max(abs(Wh)))) 
 xlabel('Omega'),ylabel('|W_h(Omega)| dB:pi/200'),axis([-pi pi -130 0]) 
 ...
 xlabel('Omega'),ylabel('|W_r| zoomed'),axis([-.4 .4 -35 0]) 

 subplot(2,1,2) 
 plot(Omega,20*log10(abs(Wh)/max(abs(Wh)))) 
 xlabel('Omega'),ylabel('|W_h| zoomed'),axis([-.4 .4 -60 0]) 
 ...
 yr1=x1.*wr;  yh1=x1.*wh; 
 Yr1=fftshift(fft(yr1,400));   Yh1=fftshift(fft(yh1,400)); 
 ...
 %% Part (e) : 
 %% ========== 
 x2=cos(26*pi*[0:M]/100) + .02*cos(51*pi*[0:M]/100); 
 yr2=x2.*wr;  yh2=x2.*wh; 
 Yr2=fftshift(fft(yr2,400));   Yh2=fftshift(fft(yh2,400)); 
